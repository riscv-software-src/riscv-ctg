...
def ldformat_opcomb(cgf, randomization):
    rs1_picked = []
    rd_picked = []
    op_comb = []
    if 'rs1' in cgf:
...
def ldformat_valcomb(cgf,op_node,randomization):
    val_comb = []
...
def ldformat_inst(op_comb, val_comb, cgf,op_node):
...
def ldformat_testreg(instr_dict):
    total_instr = len(instr_dict)
    available_reg = default_regset.copy()
    available_reg.remove('x0')
    count = 0
...
def ldformat_correct_val(instr_dict, op_node):
    return instr_dict
