...
#                    value = '0x' + struct.pack(size,value).hex()
    ...
    def write_test(file_name,node,label,instr_dict, op_node, usage_str):
        ...
        TEST_CODE_BEGIN = "RVETEST_CODE_BEGIN" if 'e' in base_isa else "RVTEST_CODE_BEGIN"
        count = 0
        for instr in instr_dict:
        ...
        with open(file_name,"w") as fd:
            fd.write(usage_str + test_template.safe_substitute(data='\n'.join(data),test=test,sig='\n'.join(sign),isa="RV"+str(xlen)+op_node_isa,TEST_CODE_BEGIN=TEST_CODE_BEGIN,opcode=opcode,extension=extension,label=label))
